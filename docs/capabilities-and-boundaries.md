...
| mode | 定位 | 默认倾向 |
| --- | --- | --- |
| `guarded` | Read Only | 只读和搜索优先，写入、bash、后台任务、网络、破坏性操作更严格 |
| `standard` | Ask First | 日常使用，常见危险类别需要确认 |
| `trusted` | Edit Freely | 信任当前项目，写入和 bash 更顺畅，后台、网络、破坏性操作仍保守 |
| `sovereign` | Full Auto | sandbox 内尽量自动执行，破坏性操作仍保留确认 |
...
`/allow <category>` 和确认框里的限时允许只会放开当前策略为 `ask` 的类别，不能覆盖 `deny`。这避免平台用户通过一句命令突破配置里的禁止策略。
