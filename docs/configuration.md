...
plugins:
  ...
  runtime:
    ...
    restart_failure_window_seconds: 300
...
Worker 默认没有网络权限，也不能直接读取宿主工作区。需要文件、MCP、长期进程或开发目录的插件应声明资源，由宿主端口校验并代理。每个激活 generation 拥有独立 Worker；热重载先启动候选 Worker，成功切换能力快照后才回收旧 generation。Worker 意外退出后按 `restart_backoff_seconds` 重建；在 `restart_failure_window_seconds` 内达到 `restart_failure_limit` 会打开 circuit breaker，后续调用失败关闭并等待显式重载或重启。
...
| `process_backend` | `auto` / `bwrap` / `appcontainer` / `legacy`；Linux/WSL 的 `auto` 使用 Bubblewrap，原生 Windows 的 `auto` 使用 AppContainer Shell Broker 启动 PowerShell 7；`legacy` 是需要显式选择的受控宿主降级模式；MCP 保留原生命令兼容启动 |
| `file_max_write_bytes` | `file_write` 单次最大写入字节数 |
| `audit_enabled` | 是否记录工具审计日志到 `data/audit.log` |

未显式设置 `storage.data_dir` 时，原生 Windows 使用 `%LOCALAPPDATA%\\LunaAgent` 作为运行数据、默认 Bash 工作目录和默认可写沙箱根；Linux/WSL 继续使用项目下的 `./data`。已有配置中的显式路径不会被迁移。
