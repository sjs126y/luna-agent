...
<h1>Backend Progress</h1>

<p><strong>Runtime、工具、安全、记忆、平台与 Delivery 的后端进度</strong></p>
...
阶段验证：Worker/RPC/Bubblewrap、Codex Bridge 与外置插件兼容测试通过；聚焦回归 `91 passed`，完整回归 `1251 passed, 1 warning`，唯一警告仍来自飞书 SDK 的既有弃用 API。
...
- 新增独立 workspace 包 `luna-agent-plugin-sdk`，承载 Plugin API 版本、manifest/dependency、Tool、Hook、Command、Active 与 RuntimeContext 公共契约；宿主旧路径只保留兼容重导出。
- manifest 新增 `plugin_api` 与 `requires`：支持 Luna Agent/SDK 版本、插件版本、宿主 capability 和 MCP tool 依赖；插件按拓扑加载，缺失/禁用/版本冲突/循环进入 `BLOCKED`，卸载会保护启用中的 dependents。
...
- 新增 generation-bound `ctx.resources`：Tool/MCP 精确 allowlist、LLM、Conversation、Delivery、Artifact 与 revisioned Storage；主动 Tool/MCP 仍经过现有 Hook、硬安全、Executor 和 audit，写入、Shell、后台进程、代码执行与 destructive 工具硬拒绝。
...
- 核心 `luna-agent plugins` CLI 与运行中 `/plugins` 命令共享控制面；Runtime health 新增 snapshot revision、lease、generation、安装状态和 pending removal 诊断。
...
- 当前完整验证：`python -m compileall -q src/luna_agent` 通过；`uv run pytest -q` 为 `1050 passed, 1 warning`，唯一警告来自飞书 SDK 的弃用 API。
- 文档清理提交 `f3da3d7` 后的 Git tracked Python 规模为 325 个文件、75,811 行：`src/luna_agent` 47,458 行，tests 27,549 行，项目插件 488 行，scripts/examples 316 行。
...
- README 从内部实现说明改为产品展示页：首屏品牌、Shields 状态标签、功能卡片、平台矩阵、典型任务、安全模式、快速开始和文档导航。
- 新增 `docs/README.md` 文档中心，按“了解产品、开始使用、参与开发、查看历史”组织阅读路线。
- 面向读者的主文档统一 Hero、状态标签、导航栏、摘要表和 Mermaid；Architecture、Progress 与 Roadmap 的长篇历史/实现内容使用可展开区域降低首屏噪音。
...
- 阶段提交：`53220ee`。
- 最终验证：`python -m compileall -q src/luna_agent` 通过；全量回归 `1050 passed`，仅有原有飞书 SDK 弃用警告。

<details>
<summary><strong>展开历史后端进度（2026-07-17 及更早）</strong></summary>
...
- 修复 Luna Agent/Qdrant UUID 表示差异导致的写入后无法召回：Qdrant 返回 ID 在适配边界统一为 SQLite 使用的 UUID hex；已有记忆无需清空或重建。默认 Luna Agent 所需 `qdrant-client` 改为基础依赖。
...
- 最终验证：`982 passed`；`python -m compileall -q src/luna_agent`、`git diff --check` 和真实 `luna-agent doctor --json --section runtime` 通过，8 个启用 MCP server 全部 ready。
...
- 最终全量回归 `967 passed`；`compileall`、`git diff --check` 和真实 `luna-agent doctor` 通过，配置正常、Runtime 已就绪。
...
- Hook 固定 Codex 新线程的项目 cwd、`workspace-write` sandbox 和 `never` 内层审批，移除调用方传入的扩权 config；Luna Agent 外层 MCP 工具审批仍保留。`codex-reply` 可继续插件隔离目录中的线程。
...
- 新增独立 `luna_agent.hooks.HookManager`，由 AppRuntime 持有；PluginManager 只负责注册转发、owner 归属和插件卸载清理。
...
- 修正 doctor 对 Streamable HTTP MCP 的误报；普通 `uv run luna-agent doctor --verbose` 已验证 GitHub `runtime=ready`、44 个工具，Luna Agent memory 正常，不再需要 `--env-file` 启动参数。
- 测试隔离补充：runtime 单测显式关闭真实 external memory，避免继承本机 Luna Agent/Qdrant 配置。

验证：配置/MCP/memory/runtime/doctor 聚焦回归 `139 passed`；全量回归 `868 passed`；`python -m compileall -q src/luna_agent` 与 `git diff --check` 通过。

## 2026-07-13：真实联调收尾

当前状态：本轮后端修复已合并回 `main`，合并提交为 `daaadf3 Merge agent tool loop and memory recovery fixes`。合并前完成 `python -m compileall -q src/luna_agent`、`git diff --check` 和全量测试，结果为 `864 passed`。
...
- fallback observation 可由 `MemoryReviewService` 后台批量迁回 Luna Agent，不占用 Agent 主循环；迁移失败保留 attempts/error，后续继续恢复。
- Luna Agent 会探测并修复 Qdrant collection/vector/payload index；本轮真实运行中 embedding 与 Qdrant query 均成功，当前微信 scope 保持 `lumora`，已有 pending observation 成功迁移。
...
- GitHub MCP 的 `headers_env` 当前由 connection 直接读取 `os.environ`，与项目统一的 `ConfigLoader -> Settings` 边界不一致。普通 `uv run luna-agent serve` 因此无法使用 `.env` 中的动态 MCP header；临时可用 `uv run --env-file .env luna-agent serve` 启动。
...
- MCP client 切换到官方 Python SDK 稳定 v1.x，协议与 transport 封装在 Luna Agent connection contract 后，保留旧 stdio 配置兼容。
...
- 根目录新增 `luna-agent-roadmap.zh-CN.md`，将后续架构路线图整理为中文；移除已完成的 provider/transport 实施指令和前端历史清理计划，并修正 `CLAUDE.md` 的过时链接与 provider/transport 描述。
...
- Project display rename：对外展示名从 `Luna Agent` 调整为 `Luna Agent`；内部 Python 包名 `luna_agent` 和 CLI 命令 `luna-agent` 暂时保留，避免破坏运行入口。
...
- Doctor output v1：`luna-agent doctor` 默认改为普通用户摘要，只展示状态、模型、运行时、配置、记忆、MCP、工具、网关、平台、插件、最多 5 条注意事项和下一步；新增 `doctor --verbose` 保留原完整开发诊断，`--json` / `--section` 行为不变。
...
- 新增 `luna_agent.platforms.attachments` 公共 helper，统一 kind 归一化和 `url/local_path/platform_file_id` 归类。
...
```bash
python -m compileall -q src/luna_agent tests/test_platform_adapters.py
...
- v1：新增 `luna_agent.activity` 聚合层，统一 summary/list/detail 结构；后台进程工具增加 `process_snapshot(...)`、`process_detail(...)`、`process_choices(...)`；runtime health 增加 `activity`。
...
- `build_context_budget(...)` 和 `luna-agent tokens session` 会使用显式上下文窗口。
...
- Luna Agent 返回实际落库后的 `memory_id/content/previous_content`，并记录 search、resolve、apply、embedding、Qdrant 各阶段耗时。
...
- memories schema v4 记录逐条索引尝试次数、最近错误和更新时间；Luna Agent worker 每次后台修复一条 pending index。
...
- `97e1dfe Probe and repair Luna Agent memory health`

真实配置验证：probe 状态为 `ok`，Luna Agent 可用；诊断准确报告全局 migration pending `59`、index pending `1`，未在前台自动迁移这些数据。
...
```bash
python -m compileall -q src/luna_agent plugins/github_assistant
...
## 2026-07-17：Luna Agent 内部检索 Backend

状态：已完成实现并通过全量回归与真实远程环境验证。

- Luna Agent 外部记忆提供器内部拆分为 embedding、vector、keyword、fusion 与可选 reranker Backend，通过轻量 Registry 和工厂组装；公共接口不包含 Qdrant、PostgreSQL 或本地文件数据库的专属字段。
...
- 配置迁入 `memory.providers.luna`，删除 embedding `api_mode` 和公共 Qdrant 配置；只校验当前选中 Backend，密钥仍统一通过 `Settings` 解析。
...
- 新增 `luna-agent memory reindex --index all|vector|keyword`，并扩展 memory doctor 的组件状态、fingerprint、generation 与分索引 backlog。

阶段提交：

- `c430b58 Define Luna Agent backend contracts`
- `7841415 Configure Luna Agent retrieval backends`
- `97cd71d Track Luna Agent backend indexes`
...
python -m compileall -q src/luna_agent
uv run pytest -q
uv run luna-agent memory doctor --json
```

结果：全量回归 `989 passed`。真实 Memory doctor 使用现有百炼 Embedding 与远程 Qdrant 探测成功，Luna Agent 保持主提供器，embedding/vector/keyword/fusion 均为 ready、reranker 为 disabled；102 条现有记忆的 vector 与 keyword 索引均为 ready，全局索引 pending 为 0。

后续已将当前 `config.yaml` 切换为 Qdrant Local `./data/memory/qdrant`。全量重建增加按 Embedding Backend 限制的批处理；百炼 `text-embedding-v4` 使用每批 10 条，避免超出接口批量上限。现有 Archive 的 102 条记忆已全部迁移到本地 Qdrant，真实 doctor 显示本地 fingerprint、102 条 vector ready、0 pending，Luna Agent 未 fallback。远程 Qdrant collection 未删除。
...
- 出站媒体从宿主 `file://` URI 改为 `base64://` segment，解决 Luna Agent 在 WSL、NapCat 在 Windows 时路径不可见的问题，并避免把本地路径交给协议端。
...
- 以完整 Handoff Summary 替换 300 字滚动摘要，用户原话和近期完整消息按 token 构建 replacement history。
- 摘要不设置压缩专属输出上限；旧 `compression.max_tokens` 已废弃并忽略。
- checkpoint 在主模型调用前独立写入新物理 Session，mid-turn 工具循环可继续创建窗口并即时落盘。
- SQLite 保存窗口 lineage、触发原因和压缩前后 token；新增 `/compact` 与 `/usage` 窗口诊断。

阶段提交：

- `06b1061 Implement handoff context compaction`
- `cef36ec Persist compaction checkpoints before turns`
- `57cc3b4 Checkpoint context during tool loops`
- `ea11c7d Record compaction windows and diagnostics`

验证：`luna-agent doctor --json` 为 `ok=true` 且无配置警告；全量回归 `1185 passed, 1 warning`，警告仍为飞书 SDK 的既有弃用提示。

### Provider 与模型能力解析

- 新增集中式 Provider/Model capability catalog，区分模型硬上限、Luna Agent 有效上下文、输出上限及数据来源。
- OpenAI 未显式配置时使用 256K 经济窗口；其他已知 provider 使用模型硬上限；未知模型保守回退 256K，显式配置不会越过已知硬上限。
- OpenRouter 仅在静态 catalog 未命中时查询模型元数据，使用 3 秒超时和 24 小时本地缓存，失败不影响 Agent 创建。
- `LLM_API_MODE=auto` 改为 provider 默认优先，主 Agent、Memory LLM、插件 LLM port 和视觉 LLM 使用同一结果。
- 自动压缩阈值提升到 0.9，并统一预留输出 token 与 1%/4096 token 安全余量。
- `doctor`、`/usage`、LLM 事件和 turn report 增加协议、上下文、输出限制、来源与裁剪状态。

阶段提交：

- `7c6f345 Add provider and model capability catalogs`
- `e595c53 Resolve effective model context limits`
- `3d57322 Improve automatic API mode selection`
- `37994da Expose model capability diagnostics`

验证：当前 `gpt-5.6-terra` / `codex_responses` 配置经 `doctor --json` 解析为 256K 有效窗口和 1.05M 模型硬上限，配置无错误或警告；全量回归 `1197 passed, 1 warning`，警告仍为飞书 SDK 的既有弃用提示。

### DeepSeek Anthropic 默认协议

- DeepSeek provider 的 `auto` 默认协议改为 `anthropic_messages`。
- 官方根地址在 `auto/anthropic_messages` 下规范化为 `https://api.deepseek.com/anthropic`；显式 `chat_completions` 时规范化回普通官方根地址。
- `.env.example`、CLI 初始化模板、配置文档和后端协议契约统一采用 Anthropic Messages 示例。

验证：Provider、配置、CLI 与 Transport 聚焦回归 `117 passed`；全量回归 `1201 passed, 1 warning`，警告仍为飞书 SDK 的既有弃用提示。

### Codex App Server 插件开发桥

- 新增外部开发工作区和每插件唯一 Codex Thread 的持久模型；创建时生成插件脚手架、`PLUGIN_BRIEF.md` 与 `LUNA_PLUGIN_DEVELOPMENT.md` 规范副本。
- 新增异步 `plugin_dev_create/message/list/status/events/cancel` 工具。消息在当前 Turn 忙碌时进入有界队列，Codex 事件只保留最近 50 条并可查询最近 20 条。
- 新增 `codex_approval_list/decide`，默认 `on-request + user`，允许一次或拒绝；重启、卸载和 generation 变化不自动批准。
- App Server 事件通过 `ActiveConversationIntent` 交给配置的 `notify_sessions`，按 `turn_started`、`assistant_message`、`approval_requested`、`error` 等事件使用不同提示词，不伪造普通用户消息。
- 旧 `mcp__codex__codex` MCP 与 Hook 继续保留；Codex Thread 恢复不静默续跑中断 Turn。

阶段提交：

- `d4cdf9c docs: define Codex plugin development contract`
- `f612a82 feat: add asynchronous Codex plugin development bridge`
- `0b395e0 fix: bound Codex event delivery and turn queue state`
- `c014ed9 fix: distinguish Codex approvals from input requests`

验证：Codex Bridge、开发模型、持久化、事件和审批聚焦回归 `8 passed`；全量回归 `1220 passed, 1 warning`；`python -m compileall -q plugins/codex_bridge` 通过。
