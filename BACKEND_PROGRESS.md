...
- 完整回归：`1112 passed, 1 warning`；唯一警告仍来自飞书 SDK 的弃用 API。
- 新增首个实际主动插件 `integrations/workspace-watch`：通过受限 `file_info` 建立工作区文件基线，等待变化稳定后合并提交给主 Agent；签名存入 revision storage，默认关闭且不预填真实 session。
