...
class PluginDependencies:
    ...
    def from_mapping(cls, value: Any) -> "PluginDependencies":
        ...
        unknown = sorted(set(value) - {"lumora", "sdk", "plugins", "capabilities", "mcp_tools"})
        if unknown:
            raise ValueError("Unknown plugin dependency field(s): " + ", ".join(unknown))
        lumora = str(value.get("lumora") or "").strip()
        sdk = str(value.get("sdk") or "").strip()
        _validate_specifier(lumora, "requires.lumora")
        ...
        return cls(lumora, sdk, tuple(plugins), capabilities, mcp_tools)
