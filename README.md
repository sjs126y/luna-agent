...
| 不同使用场景需要不同自动化程度 | `guarded` / `standard` / `trusted` / `sovereign` 四档 mode 覆盖只读、询问优先、项目内放权和全自动。 |
...
危险操作不会直接裸露给模型。`guarded`、`standard`、`trusted`、`sovereign` 四档 execution mode 决定默认自动化程度；所有工具统一经过 execution guard、precheck、permission、sandbox 和 audit。CLI/TUI 与 Gateway 都可以复用确认语义，包括允许一次、拒绝、限时允许和 pending confirmation。
...
- mode 与 permission policy 决定工具能否自动执行，`/allow` 只能放开当前策略为 `ask` 的类别，不能覆盖 `deny`。
