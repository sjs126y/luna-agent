...
from luna_agent.plugins.builtin.memory.luna.backends.base import BackendHealth
from luna_agent.plugins.builtin.memory.luna.backends.config import RetrievalConfig
from luna_agent.plugins.builtin.memory.luna.backends.keyword import SqliteFts5KeywordIndex
from luna_agent.plugins.builtin.memory.luna.backends.ranking import NoOpReranker, WeightedRrfFusion
from luna_agent.plugins.builtin.memory.luna.provider import LunaMemoryProvider
from luna_agent.plugins.builtin.memory.luna.qdrant_store import QdrantMemoryIndex, QdrantVectorConfig
...
async def test_qdrant_index_creates_filter_indexes_for_existing_collection() -> None:
    ...
    index = QdrantMemoryIndex(
        QdrantVectorConfig(collection="luna_memories", url="http://qdrant"), dimensions=1024, client=client
    )
    ...
    assert client.created_indexes == [
        ("luna_memories", "user_id", "keyword", True),
        ("luna_memories", "profile", "keyword", True),
    ]
...
@pytest.mark.asyncio
async def test_luna_migrate_returns_applied_memory_id(tmp_path) -> None:
    ...
    assert stored.provider == "luna"
    assert stored.metadata["index_status"] == "ready"
    await archive.close()


@pytest.mark.asyncio
async def test_luna_reindexes_pending_records(tmp_path) -> None:
    archive = MemoryArchive(tmp_path / "memory.db")
    await archive.initialize()
    scope = MemoryScope(user_id="u1")
    record = MemoryRecord(
...
@pytest.mark.asyncio
async def test_luna_full_reindex_batches_embedding_and_vector_writes(tmp_path) -> None:
    archive = MemoryArchive(tmp_path / "memory.db")
    await archive.initialize()
    scope = MemoryScope(user_id="u1")
...
@pytest.mark.asyncio
async def test_luna_search_recalls_uuid_returned_by_qdrant(tmp_path) -> None:
    archive = MemoryArchive(tmp_path / "memory.db")
    await archive.initialize()
    scope = MemoryScope(user_id="u1")
    ...
    provider = _provider(
        ...
        vector_index=QdrantMemoryIndex(
            QdrantVectorConfig(collection="luna_memories", url="http://qdrant"),
            dimensions=2,
            client=qdrant,
        ),
    )
...
@pytest.mark.asyncio
async def test_luna_search_degrades_to_keyword_when_vector_fails(tmp_path) -> None:
    archive = MemoryArchive(tmp_path / "memory.db")
    await archive.initialize()
    scope = MemoryScope(user_id="u1")
