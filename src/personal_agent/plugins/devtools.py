...
from lumora_plugin_sdk import HookEvent, PLUGIN_API_VERSION, SDK_VERSION, PluginManifest
from lumora_plugin_sdk.testing import run_plugin_contract

from personal_agent.plugins.dependencies import HOST_CAPABILITIES
...
def schema_document(kind: str) -> dict[str, Any]:
    ...
    if name in {"create", "scaffold", "scaffold-spec"}:
        return {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "title": "Lumora Plugin Scaffold Spec",
            "type": "object",
            "required": ["key", "name"],
            "properties": {
                "key": {"type": "string"},
...
def _module_template(features: set[str], hook_events: list[str]) -> str:
    imports = ["from lumora_plugin_sdk import PluginRuntimeContext"]
    if "command" in features:
        imports[0] = "from lumora_plugin_sdk import CommandEntry, PluginRuntimeContext"
    if "tool" in features:
    ...
    lines = ["\"\"\"Lumora external plugin entrypoint.\"\"\"", "", "from __future__ import annotations", "", *imports, ""]
    if "active" in features:
        lines.extend([
...
- Import public contracts from `lumora_plugin_sdk`; do not import host internals from `personal_agent`.
