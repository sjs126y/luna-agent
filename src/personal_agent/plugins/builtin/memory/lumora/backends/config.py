"""Provider-neutral configuration for Lumora retrieval backends."""
...
class LumoraBackendConfig:
    ...
    @classmethod
    def from_options(cls, value: dict[str, Any] | None) -> "LumoraBackendConfig":
...
def _positive_float(data: dict[str, Any], key: str, default: float) -> float:
    ...
    if value <= 0:
        raise ValueError(f"memory.providers.lumora.retrieval.{key} must be positive")
    return value
