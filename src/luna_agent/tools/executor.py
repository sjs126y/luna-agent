...
from luna_agent.tools.execution_guard import (
    GuardDecision,
    ToolDecision,
...
from luna_agent.tools.entry import ToolArtifact, ToolHandlerOutput
    ...
    *,
    ...
    try:
        ...
        if batch_confirmation.security_context is not None and (
            ...
            from luna_agent.security.evaluator import revoke_grants
...
async def execute_tool_call_result(
    ...
    *,
    ...
    async def _finish(result: ToolExecutionResult) -> ToolExecutionResult:
        ...
        try:
            from luna_agent.tools.audit import audit_tool_result
    ...
    if artifact_store is not None and materialized_artifacts and nested_result is None:
        from luna_agent.artifacts import ArtifactStoreError, materialize_tool_artifact
...
def _would_need_permission_confirm(tc: dict, entry: Any, agent: Any = None) -> bool:
    from luna_agent.security.evaluator import (
...
async def _emit_tool_decision(event_sink: Any, decision: ToolDecision) -> None:
    try:
        from luna_agent.tools.audit import audit_tool_decision
