"""Luna long-term memory with LLM resolution and hybrid retrieval."""
...
from luna_agent.memory.prompts import MEMORY_RESOLUTION_SYSTEM

logger = logging.getLogger(__name__)


class LunaMemoryProvider(ExternalMemoryProvider):
    name = "luna"
    ...
    async def search(self, query: str, scope: MemoryScope, *, limit: int = 5) -> list[MemoryRecord]:
        ...
        if not result_sets:
            ...
            raise RuntimeError(f"Luna retrieval backends failed: {detail}")
        ...
        try:
            ...
            logger.warning("Luna reranker degraded to fusion results: %s", self._component_errors["reranker"])
            ranked = fused[:limit]
        self.last_error = ""
        return [
            replace(
    ...
    async def migrate(self, observations, scope: MemoryScope) -> MemoryReviewResult:
        ...
        for observation in observations:
            ...
            logger.info(
                "Luna memory migrate: action=%s search=%.3fs resolve=%.3fs apply=%.3fs total=%.3fs",
    ...
    async def reindex(self, records: list[MemoryRecord], scope: MemoryScope) -> dict[str, int]:
        ...
        for record in records:
            ...
            try:
                ...
                result["failed"] += 1
                logger.warning("Luna reindex deferred: %s", self.last_error)
                continue
    ...
    async def _apply_change(self, scope, observation, change, related) -> MemoryChange:
        ...
        try:
            ...
            logger.warning("Luna index write pending: %s", self.last_error)
            return applied
    ...
    async def _write_indexes(self, record: MemoryRecord, *, indexes: set[str] | None = None) -> None:
        ...
        finished_at = monotonic()
        logger.info(
            "Luna index write: embedding=%.3fs qdrant=%.3fs",
            upserted_at - embedded_at,
