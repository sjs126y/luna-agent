"""Built-in Luna backend registrations and assembly."""
...
def build_luna_backends(*, context, archive) -> LunaBackends:
    config = LunaBackendConfig.from_options(context.provider_options)
...
def validate_luna_backends(context) -> list[str]:
    try:
        config = LunaBackendConfig.from_options(context.provider_options)
...
def _create_qdrant(*, selection: BackendSelection, context, **kwargs):
    ...
    return QdrantMemoryIndex(QdrantVectorConfig(
        ...
        collection=str(options.get("collection") or "luna_memories"),
