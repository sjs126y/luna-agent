...
from luna_agent.attachments import DownloadedAttachment
from luna_agent.attachments.store import DEFAULT_MAX_BYTES
...
from luna_agent.models.messages import AttachmentRef, MessageEnvelope, MessageEvent, MessagePart, SessionSource
...
class WeChatAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        if not self._token or not self._account_id:
            error = "WeChat not logged in. Run: uv run luna-agent setup --platform wechat"
        ...
        for _ in range(480):
            ...
            if state == "confirmed":
                ...
                if qr_data:
                    ...
                    refresh_count += 1
                    if refresh_count > 3:
                        print("\n❌ 二维码多次过期，请重新运行 luna-agent setup --platform wechat。")
                        return None
