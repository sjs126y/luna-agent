...
from luna_agent.security.modes import MODE_PRESETS
...
VALID_EXTERNAL_MEMORY_PROVIDERS = {"none", "fallback", "luna", "lumora", "mem0"}
...
def _validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if providers is not None and not isinstance(providers, dict):
        ...
        luna = providers.get("luna")
        legacy_lumora = providers.get("lumora")
        if luna is not None and legacy_lumora is not None:
            errors.append("memory.providers 只能配置 luna，lumora 仅作为旧配置别名。")
        selected = luna if luna is not None else legacy_lumora
        selected_key = "luna" if luna is not None else "lumora"
        if selected is not None and not isinstance(selected, dict):
            errors.append(f"memory.providers.{selected_key} 必须是对象。")
        elif isinstance(selected, dict):
            for name in ("embedding", "vector", "keyword", "fusion", "reranker"):
                selection = selected.get(name)
                if selection is not None and not isinstance(selection, dict):
                    errors.append(f"memory.providers.{selected_key}.{name} 必须是对象。")
                elif isinstance(selection, dict):
                    _string_value(
                        selection,
                        "provider",
                        f"memory.providers.{selected_key}.{name}.provider",
                        errors,
                    )
...
def _next_steps(
    ...
    steps: list[str] = []
    if not config_path.exists():
        steps.append("运行 luna-agent init 生成 config.yaml。")
    if not env_example_path.exists():
        steps.append("运行 luna-agent init 生成 .env.example。")
    if not env_path.exists():
        steps.append("根据 .env.example 创建 .env，或运行 luna-agent init --copy-env。")
    if missing_llm_env:
    ...
    if any(not item["exists"] and item["required"] and item.get("portable", True) for item in directories):
        steps.append("运行 luna-agent init --fix-dirs 创建基础目录。")
    ...
    if errors:
        steps.append("先修复上面的配置错误，再启动 Luna Agent。")
    elif warnings:
        steps.append("修复上面的配置警告后再运行 luna-agent doctor。")
    if not steps:
        steps.append("配置检查通过，可以运行 luna-agent chat 或 luna-agent serve。")
    return steps
...
def _recommended_commands(
    *,
    ...
    if not config_path.exists() or not env_example_path.exists():
        commands.append("luna-agent init")
    if not env_path.exists():
        if env_example_path.exists():
            commands.append("cp .env.example .env")
        commands.append("luna-agent init --copy-env")
    if any(not item["exists"] and item["required"] and item.get("portable", True) for item in directories):
        commands.append("luna-agent init --fix-dirs")
    if missing_llm_env:
        commands.append("编辑 .env，填写 LLM_API_KEY")
    if path_warnings:
        commands.append("编辑 config.yaml，修正 Windows/WSL 路径")
    if errors:
        commands.append("luna-agent init --check")
    if warnings:
        commands.append("luna-agent doctor")
