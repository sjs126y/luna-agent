...
from luna_agent.config_registry import effective_config_snapshot
...
_CONFIG_TEMPLATE_LOCAL = """# Luna Agent minimal configuration
agent:
  max_iterations: 50
...
def _platform_config_template(platform: str) -> str:
    return (
        ...
        .replace("# Luna Agent bot configuration", f"# Luna Agent {platform} bot configuration")
...
def chat(
    ...
    try:
        if one_shot:
            ...
            from luna_agent.tui.app import run_inline_tui_sync
...
def plugins_capabilities(
    ...
    from luna_agent.plugins.devtools import capability_catalog
...
def plugins_schema(
    ...
    from luna_agent.plugins.devtools import schema_document
...
def plugins_create(
    ...
    from luna_agent.plugins.devtools import create_plugin, load_scaffold_spec
...
def plugins_test(
    ...
    from luna_agent.plugins.devtools import contract_test
...
def plugins_diff(before: Path, after: Path) -> None:
    ...
    from luna_agent.plugins.devtools import diff_plugins
...
def tokens_estimate(
    ...
    from luna_agent.llm.token_counter import estimate_tokens
...
def agents_export(
    ...
    from luna_agent.plugins.builtin.tools.builtin.delegate import get_agent_run
...
def agents_clear() -> None:
    _load_agent_run_store()
    from luna_agent.plugins.builtin.tools.builtin.delegate import clear_agent_runs
...
def protocol_schema(
    ...
    from luna_agent.conversation.events import frontend_protocol_schema
...
def memory_reindex(
    ...
    """Rebuild Luna derived indexes from the SQLite archive."""
...
def _plugin_validation_manager(path: Path, settings: Settings | None = None) -> PluginManager:
    ...
    validation_root = Path(tempfile.mkdtemp(prefix="luna-agent-plugin-integration-"))
...
def _load_agent_run_store(settings: Settings | None = None) -> Settings:
    settings = settings or Settings()
    from luna_agent.plugins.builtin.tools.builtin.delegate import load_agent_runs
...
async def _with_app_runtime(callback):
    from luna_agent.runtime import create_app_runtime
...
async def _build_serve_dry_run_report() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "next_steps": ["先修复启动检查错误，再运行 luna-agent serve --dry-run。"],
        }
    finally:
...
def build_doctor_report(settings: Settings | None = None) -> dict[str, Any]:
    ...
    from luna_agent.tools.registry import tool_registry
    from luna_agent.tools.process_sandbox import process_sandbox_snapshot
...
def _boot_dict_from_exception(exc: BaseException) -> dict[str, Any]:
    from luna_agent.runtime import boot_report_from_exception
...
def _doctor_next_steps(report: dict[str, Any], issues: list[str]) -> list[str]:
    ...
    steps = ["uv run luna-agent chat"]
    platforms = list(report.get("platforms") or [])
    if any(item.get("enabled", True) and not item.get("missing_env") for item in platforms):
        steps.append("uv run luna-agent serve")
    if issues:
        steps.append("uv run luna-agent doctor --verbose")
    return steps
...
def _format_doctor_section(report: dict[str, Any], section: str) -> str:
    ...
    lines = [f"Luna Agent doctor: {section}"]
    if section == "runtime":
        runtime = report.get("runtime", {})
        gateway = report.get("gateway", {})
        lines.extend([
